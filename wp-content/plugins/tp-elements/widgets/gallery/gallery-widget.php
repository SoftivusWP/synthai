...
class Themephi_Elementor_pro_Gallery_Widget extends \Elementor\Widget_Base {
	...
	protected function register_controls() {
        ...
        $this->add_control(
            'gallery_messonry',
            [
                'label' => esc_html__( 'Enable Messonry', 'tp-elements' ),
                'type' => Controls_Manager::SELECT,
                'default' => 'no',
                'options' => [
        ...
        $this->add_control(
            'col_md',
            [
                'label'   => esc_html__( 'Desktops > 769px', 'tp-elements' ),
                'type'    => Controls_Manager::SELECT,  
                'default' => 2,         
                'options' => [
                    '1' => esc_html__( '1 Column', 'tp-elements' ), 
	...
	protected function render() {
		...
		$column_width = ' row-cols-xxl-'. $settings['col_xxl'] .' row-cols-xl-'. $settings['col_xl'] .' row-cols-lg-'. $settings['col_lg'] .' row-cols-md-'. $settings['col_md'] .' row-cols-sm-'. $settings['col_sm'] .' row-cols-'. $settings['col_xs'] .'  ';


		if( $settings['image_spacing'] != 'custom' ) {
			$custom_vertical_space = 'custom-vertical-space';
		} else {
			$custom_vertical_space = '';
		}
	    ...
	    <div class="row custom-vertical-space <?php echo esc_attr( $column_width ); ?> rs-galleys elementor-image-gallery <?php echo esc_html( $settings['gallery_column_gap'] );?>"  <?php if( $settings['gallery_messonry'] == 'yes' ) : ?> data-masonry='{ "columnWidth": ".grid-item", "percentPosition": false }' <?php endif; ?> > 	   			
	   		<?php
